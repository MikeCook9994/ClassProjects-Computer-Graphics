function Pipe(glHost) {
    this.glHost = glHost;
}

Pipe.prototype.SetupProgram = function() {
    ...
    let attributeData = [
        new Float32Array(pipeObjectAttributes.vertices), 
        new Float32Array(pipeObjectAttributes.vertexNormals), 
        defineBaryCentricCoordiantes(pipeObjectAttributes.vertices.length)
    ];
    this.attributeBuffers = this.glHost.BufferAttributeData(this.attributeNames, attributeData);
}

Pipe.prototype.SetUniformValues = function(cameraMatrix, projectionMatrix, isWireframe) {
...
Pipe.prototype.Draw = function() {
    this.glHost.gl.drawArrays(this.glHost.gl.TRIANGLES, 0, pipeObjectAttributes.vertices.length / 3);
}
